...
func zigRunner(params ZigExecParams) Result {
	...
	go func() {
		cmd := exec.Command("docker", []string{"exec", params.ContainerName, "/bin/sh", "-c", fmt.Sprintf("cd %s && zig run %s", params.ContainerDirectory, params.ExecutionFile)}...)
