...
func PythonSingleFileBuild(params RubySingleFileBuildParams) (PythonSingleFileBuildResult, *appErrors.Error) {
	...
	if err := os.MkdirAll(tempExecutionDir, os.ModePerm); err != nil {
		return PythonSingleFileBuildResult{}, appErrors.New(appErrors.ApplicationError, appErrors.FilesystemError, fmt.Sprintf("Cannot create execution dir: %s", err.Error()))
	}
